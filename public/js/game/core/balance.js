...
export const BALANCE = {
  ...
  /*
   * Combo 的獎勵一律是「更好打」，不是「不用打」。
   *
   * 設計書把這條寫成硬性原則：原本 Combo 10 設計成自動擊殺，那會直接少掉
   * 一次聽與一次輸入——爽度可以用時間加倍換到，學習次數不能折抵。
   * 下面三個效果全部只改「難度」，不改「要打幾個字」。
   */
  combo: {
    // 5：蜂群衝刺——三秒內敵人減速一半
    dashAt: 5,
    ...
    // 15：狂蜂狀態——五秒內擊退三倍、蜂蜜兩倍
    frenzyAt: 15,
    frenzyMs: 5000,
    frenzyKnockbackFactor: 3,
    frenzyHoneyFactor: 2,
    /*
     * 15 之後每隔這麼多再觸發一次狂蜂。
     *
     * 設計書只寫到 15。但連到 20、25 卻什麼都沒有的話，長連擊等於沒有回報，
     * 而小孩最想看到的正是「我一路沒錯」被獎勵。所以頂層重複觸發，
     * 不新增機制、也不違反上面那條原則。
     */
    frenzyRepeatEvery: 5
  },
